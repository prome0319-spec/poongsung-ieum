...
import { createClub, updateClub, deleteClub, removeMember, setMemberRole, approveClub, rejectClub } from '@/app/(main)/clubs/actions'

type PageProps = { searchParams: Promise<{ message?: string }> }

type Club = {
  id: string; name: string; description: string | null; emoji: string
  min_members: number; is_active: boolean; is_recruiting: boolean
  sort_order: number; created_at: string; status: string; proposed_by: string | null
}
...
export default async function AdminClubsPage({ searchParams }: PageProps) {
  ...
  const activeClubs = clubs.filter((c) => c.status !== 'proposed')
  ...
  return (
    <main className="page" style={{ paddingBottom: 120, display: 'grid', gap: 20 }}>
      ...
      {proposedClubs.length > 0 && (
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            <h2 style={{ margin: 0, fontSize: 15, fontWeight: 800, color: 'var(--text)' }}>승인 대기</h2>
            <span style={{ fontSize: 12, fontWeight: 700, padding: '2px 8px', borderRadius: 'var(--r-pill)', background: 'var(--warning-soft)', color: 'var(--warning)' }}>
              {proposedClubs.length}건
            </span>
          </div>
          <div style={{ display: 'grid', gap: 10 }}>
            {proposedClubs.map((club) => {
              ...
              return (
                <div key={club.id} className="card" style={{ padding: '16px', border: '1.5px solid var(--warning-border, #f59e0b)', background: 'var(--warning-soft)' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                    <span style={{ fontSize: 28, flexShrink: 0 }}>{club.emoji}</span>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 800, fontSize: 15, color: 'var(--text)', marginBottom: 4 }}>{club.name}</div>
                      {club.description && (
                        <p style={{ margin: '0 0 6px', fontSize: 13, color: 'var(--text-muted)', lineHeight: 1.5 }}>{club.description}</p>
                      )}
                      <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 8 }}>
                        관심 인원 {members.length}명 / 최소 {club.min_members}명
                        {members.length > 0 && (
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <form action={approveClub} style={{ display: 'inline' }}>
                        ...
                        <form action={rejectClub} style={{ display: 'inline' }}>
                          <input type="hidden" name="id" value={club.id} />
                          <button type="submit" style={{
      ...
      {/* 활성 동아리 목록 */}
      <div>
        <h2 style={{ margin: '0 0 12px', fontSize: 15, fontWeight: 800, color: 'var(--text)' }}>동아리 목록</h2>
